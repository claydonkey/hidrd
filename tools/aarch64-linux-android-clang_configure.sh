...
export CFLAGS="${CFLAGS} --sysroot=${SYSROOT} -I${SYSROOT}/include -I${PREFIX}/  -L${PREFIX}/sysroot/usr/include -w -Wno-error"
export CPPFLAGS="${CFLAGS}"
export LDFLAGS="${LDFLAGS} -L${SYSROOT}/lib -L${PREFIX}/lib -L${PREFIX}/sysroot/usr/lib -lz"

./configure $*  --build=x86_64-pc-mingw64  --prefix=/${PREFIX} --exec_prefix=/${PREFIX} --bindir=/${PREFIX}/bin --sbindir=/${PREFIX}/sbin --host=${CROSS_COMPILE}  --target=${CROSS_COMPILE} --with-sysroot=${SYSROOT} "$@"  --build=x86_64-pc-linux-gnu 
